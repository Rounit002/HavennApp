          ...
          <Card className="p-6 shadow-lg border-gray-200 dark:border-gray-700">
            ...
            <CardContent>
              <p className="text-gray-600 dark:text-gray-400 mb-4">
                {this.state.error?.toString() || 'An unknown error occurred while loading the schedule. Please try refreshing the page or contact support if the issue persists.'}
              </p>
              <Button
...
const SchedulePage: React.FC = () => {
  ...
  const fetchBranches = useCallback(async () => {
    try {
      ...
      // Don't auto-select a branch - let the user choose from the dropdown
      if (branchesData.length > 0) {
        setNewEvent(prev => ({
          ...prev,
          branchId: selectedBranchId || branchesData[0].id
        }));
      }
    } catch (error) {
  ...
  const fetchEvents = useCallback(async () => {
    setIsLoading(true);
    try {
      console.log('Fetching schedules for branch ID:', selectedBranchId);
      // Pass null to get all schedules when no branch is selected
      const response = await api.getSchedules(selectedBranchId || null);
      console.log('Raw API Response:', response);
      
      // Handle both response formats: { schedules: [...] } and direct array
      let schedules = [];
      ...
      const formattedSchedules = schedules
        ...
        .map((schedule: any) => {
          // Get branch name from schedule data or find it in branches list
          const branchName = schedule.branch_name || 
                           (schedule.branch_id && branches.find(b => b.id === schedule.branch_id)?.name) || 
                           '—';
          ...
          return {
            ...
            time: schedule.time || '',
            eventDate: schedule.eventDate || schedule.event_date || '',
            fee: typeof schedule.fee === 'number' ? schedule.fee : parseFloat(schedule.fee) || 0,
            branchId: schedule.branch_id,
            branchName: branchName,
          };
        });
        
      console.log('Setting events:', formattedSchedules);
  ...
  const handleAddEventSubmit = async () => {
    if (!newEvent.title.trim() || !newEvent.time || !newEvent.branchId) {
      toast.error('Please fill in all required fields: Event Title, Time, and Branch.');
      return;
    }
  ...
  const handleUpdateEventSubmit = async () => {
    ...
    if (!eventToEdit.title.trim() || !eventToEdit.time || !eventToEdit.branchId) {
      toast.error('Please fill in all required fields: Event Title, Time, and Branch.');
      return;
    }
    ...
    const scheduleDataToSend = {
      ...
      description: eventToEdit.description?.trim() || null,
      time: eventToEdit.time,
      eventDate: dateStrYYYYMMDD,
      fee: eventToEdit.fee,
      branch_id: eventToEdit.branchId,
    };

    console.log('Sending update event data:', scheduleDataToSend);
    setIsLoading(true);
    try {
      await api.updateSchedule(eventToEdit.id, scheduleDataToSend);
      await fetchEvents();
  ...
  return (
    <ErrorBoundary>
      <div className="flex min-h-screen bg-gray-100 dark:bg-gray-900">
        <Sidebar isCollapsed={isSidebarCollapsed} setIsCollapsed={setIsSidebarCollapsed} onBarcodeClick={handleBarcodeClick} />
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="w-full sm:w-64">
                    ...
                    <Select 
                      ...
                      onValueChange={(value) => {
                        const newBranchId = value === 'all' ? null : parseInt(value);
                        console.log('Selected branch ID changed:', newBranchId);
          ...
          <DialogContent className="sm:max-w-[480px] dark:bg-gray-800">
            ...
            <div className="grid gap-4 py-4">
              <div className="space-y-2">
                ...
                <Input id="eventTitle" placeholder="Enter event title" value={newEvent.title} onChange={(e) => setNewEvent({ ...newEvent, title: e.target.value })} />
              </div>
              
              <div className="space-y-2">
                <Label htmlFor="eventBranch">Branch *</Label>
                <Select 
                  value={newEvent.branchId?.toString() || ''} 
                  onValueChange={(value) => setNewEvent({ ...newEvent, branchId: parseInt(value) })}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select a branch" />
                  </SelectTrigger>
              ...
              <div className="space-y-2">
                ...
                <Input id="eventFee" type="number" placeholder="0.00" value={newEvent.fee} onChange={(e) => setNewEvent({ ...newEvent, fee: parseFloat(e.target.value) || 0 })} />
              </div>
              
              <div className="space-y-2">
                <Label htmlFor="eventDescription">Description (optional)</Label>
                <Textarea id="eventDescription" placeholder="Enter event description" value={newEvent.description || ''} onChange={(e) => setNewEvent({ ...newEvent, description: e.target.value })} />
              </div>
              
              <div className="text-sm text-gray-500 dark:text-gray-400">
                <Label>Date</Label>
                <div>{formatDateForDisplay(newEvent.date)}</div>
              </div>
            </div>
            <DialogFooter>
            ...
            {eventToEdit && (
              <div className="grid gap-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input id="editEventTitle" placeholder="Enter event title" value={eventToEdit.title} onChange={(e) => setEventToEdit({ ...eventToEdit, title: e.target.value })} />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="editEventBranch">Branch *</Label>
                  <Select 
                    value={eventToEdit.branchId?.toString() || ''} 
                    onValueChange={(value) => setEventToEdit({ ...eventToEdit, branchId: parseInt(value) })}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Select a branch" />
                    </SelectTrigger>
                ...
                <div className="space-y-2">
                  <Label htmlFor="editEventFee">Fee</Label>
                  <Input id="editEventFee" type="number" placeholder="0.00" value={eventToEdit.fee} onChange={(e) => setEventToEdit({ ...eventToEdit, fee: parseFloat(e.target.value) || 0 })} />
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="editEventDescription">Description (optional)</Label>
                  <Textarea id="editEventDescription" placeholder="Enter event description" value={eventToEdit.description || ''} onChange={(e) => setEventToEdit({ ...eventToEdit, description: e.target.value })} />
                </div>
                
                <div className="text-sm text-gray-500 dark:text-gray-400">
                  <Label>Date</Label>
                  <div>{formatDateForDisplay(eventToEdit.eventDate)}</div>
                </div>
              </div>
            )}
            <DialogFooter>
